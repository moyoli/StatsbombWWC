...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 29 11:26:40 2020
...
#reverse direction of play in 2nd half so home team is always attacking from right to left
tracking_home,tracking_away,events = mio.to_single_playing_direction(tracking_home,tracking_away,events)

#making a movie of the home team goal
PLOTDIR = DATADIR
...
#calculate minutes played for each player: TIME
minutes = []
for player in home_players:
    #search for first and last frames with position observation for each player
    column = 'Home_' + player + '_x' #using player x -position coordinate
...
#Calculate total distance for each player
distance = []
for player in home_summary.index:
    column = 'Home_' + player + '_speed'
    player_distance = tracking_home[column].sum()/25./1000 #sum of distance travelled from one observation to another
...
#create physical summary df for away players
...
#calculate minutes played for each away player: TIME
minutes = []
for player in away_players:
    #search for first and last frames with position observation for each player
    column = 'Away_' + player + '_x' #using player x -position coordinate
    player_minutes = ( tracking_away[column].last_valid_index() - tracking_away[column].first_valid_index() + 1)/25/60.
    minutes.append(player_minutes)
away_summary['Minutes Played'] = minutes
away_summary = away_summary.sort_values(['Minutes Played'], ascending=False)

#Calculate total distance for each away player
distance = []
for player in away_summary.index:
    column = 'Away_' + player + '_speed'
    player_distance = tracking_away[column].sum()/25./1000 #sum of distance travelled from one observation to another
    distance.append( player_distance )
away_summary['Distance [km]'] = distance

print(3+4)

#make a simple bar chart of distance covered for each players
plt.subplots()
ax = home_summary['Distance [km]'].plot.bar(rot=0)
ax.set_xlabel('Player')
ax.set_ylabel('Distance Covered [km]')

#plot positions at KO (to find out each player position)
